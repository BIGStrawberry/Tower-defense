...
void EnemyDataContainer::load()
{
	...
	body.setTexture(TextureContainer::get("tails.png"));
	container.insert(std::pair<EnemyType, EnemyData>(EnemyType::Boss_Flying, EnemyData{500, 0.33f, 3, 100, body}));

	body.setScale(sf::Vector2f(1.5f, 1.5f));
	body.setTexture(TextureContainer::get("tank.png"));
	container.insert(std::pair<EnemyType, EnemyData>(EnemyType::Boss_Tank, EnemyData{1050, 0.33f, 3, 100, body}));

	body.setScale(sf::Vector2f(1.5f, 1.5f));
	body.setTexture(TextureContainer::get("sonic.png"));
	container.insert(std::pair<EnemyType, EnemyData>(EnemyType::Boss_Fast, EnemyData{375, 1, 3, 100, body}));

	body.setScale(sf::Vector2f(3.5f, 3.5f));
	body.setTexture(TextureContainer::get("knuckle.png"));
	container.insert(std::pair<EnemyType, EnemyData>(EnemyType::Ubeah_Knucklez, EnemyData{150000, 0.05f, 5, 1000, body}));
}
